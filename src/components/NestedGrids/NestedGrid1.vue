  ...
  <!-- <div class="nested-grid">
   ...
   <b-table
      ...
      <template v-slot:cell(defBacklog)="row">
        <span @click="$emit('defbacklog-clicked', row, 1)" style="cursor:pointer">{{ formatNumber(row.value) }}</span>
      </template>
      ...
      <template v-slot:row-details="row">
        <NestedGrid2
          ...
          :dataAPI="row.item.childAPI" :class="generateClassId" />
      </template>
...
export default {
  ...
  components: {
    NestedGrid2: () => import('./NestedGrid1.vue'),
  },
  props: {
    dataApi: {
      type: String,
  ...
  mounted() {
    console.log('mounted', this.parentLevel, this.level);
    if (this.autoDrill) {
      this.collapseExpand();
    }
  },
